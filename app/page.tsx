  ...
  return (
    <main className="min-h-screen px-5 py-6 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        ...
        <form className="grid gap-4 border-y border-[var(--line)] py-5 md:grid-cols-[1fr_auto] md:items-end" onSubmit={handleOutlineSubmit}><div><label className="field-label" htmlFor="topic">What technical subject should become a book?</label><textarea className="field-input min-h-32 resize-y" id="topic" value={topic} onChange={(event) => setTopic(event.target.value)} onKeyDown={handleTopicKeyDown} placeholder="e.g. Vector Databases and RAG" maxLength={10000} required /><div className="mt-2 text-right text-xs text-[var(--muted)]" aria-live="polite">{topic.length.toLocaleString()} / 10,000 characters</div></div><button className="primary-button" type="submit" disabled={loading}>{loading ? "Building outline..." : "Generate outline"}</button></form>
